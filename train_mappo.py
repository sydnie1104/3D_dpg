# train_mappo.py - MAPPO多智能体训练脚本
"""
Multi-Agent PPO: On-policy算法
每个episode结束后更新一次
"""
import torch
...
def train():
    ...
    best_model_path = f"{model_dir}/{model_name}_best.pth"
    ...
    try:
        for episode in range(1, Config.TOTAL_EPISODES + 1):
            ...
            while not done:
                # 选择动作（on-policy采样）
                actions, log_probs, values = agent.select_actions(obs_list, eval_mode=False)
                ...
                # 存储经验
                agent.push_experience(obs_list, actions, rewards, log_probs, values, dones)
                
                obs_list = next_obs_list
                episode_step += 1
                done = all(dones) or info['all_done']
            
            # Episode结束，更新网络（on-policy）
            actor_loss, critic_loss, entropy = agent.update(next_obs_list)
            ...
            # 定期保存
            if episode % 100 == 0:
                agent.save_model(f"{model_dir}/{model_name}_ep{episode}.pth")
            
            # 全部成功时保存
            if is_all_success:
                timestamp = time.strftime("%Y%m%d_%H%M%S")
                success_path = f"{model_dir}/{model_name}_allsuccess_ep{episode}_{timestamp}.pth"
        ...
        print(f"所有Agent都成功: {all_success_count}")
